...
* `mode`: determines the parsing mode. By default it is PERMISSIVE. Possible values are:
  ...
  * `SUPERPERMISSIVE`: Adds extra column '__errors' of type StructType(which has two struct fields named 'row', 'error_message').
    __errors will not be null only if row is malformed.
